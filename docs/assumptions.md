...
### A64 — Structural incompatibility of independent edge pruning (V4 prune/regrow family): FEASIBILITY REJECT, not a BILUH hypothesis FAIL — recorded as a mechanism-level result, `null_results/20260818-v4-prune-regrow-feasibility.md` (2026-08-18)

**User's own synthesis of `[A57]`-`[A63]`, adopted verbatim as the
closing finding for this branch of V4:**

> In the tested adaptive network, the correlation field generated by
> the fast dynamics produces edge candidate sets so concentrated (Gini
> ≈0.98, a small number of nodes carrying 20%+ of all prune-desirable
> edges) that an INDEPENDENT edge-pruning policy cannot simultaneously
> satisfy the pre-registered structural-exposure target and bounded
> local topological turnover. Exact capacity optimization (`[A61]`,
> `[A63]`, `scipy.optimize.milp`, not a heuristic) confirms the deficit
> is a property of the FEASIBLE SET itself, not of the greedy
> implementation — the greedy selector reached 94-98% of the true
> mathematical optimum in both tested cap variants, and even that
> optimum tops out at 30-52% exposure against a 95% requirement.

**Chain of evidence that established this, each step independently
verified, not merely asserted:**

1. `[A57]` — original unconstrained rule: 100% disconnection at spec
   `ρ`, window 2, universally.
2. `[A58]` — feasibility-only diagnostic: one node loses all 6 edges in
   a single window, on every seed, at the PRUNE sub-step specifically.
3. `[A59]` — permutation-equivariance red-team test: concentration
   confirmed GENUINE (15/15 exact match after relabeling), not a
   tie-break/label artifact.
4. `[A60]` — V4-K1c (bounded current-degree cap, `q=1/2`): delays
   disconnection (window 2 → windows 10-17) but does not prevent it;
   severe under-exposure (`ICE-1=0.254`); degree drift via uncapped
   regrowth identified as the specific feedback mechanism.
5. `[A61]` — exact capacity audit on K1c: `H-B` (algorithmic weakness)
   REJECTED, `H-A` (structural incompatibility) CONFIRMED — `CR*≈0.30`.
6. `[A62]` — V4-K1d (reference-degree cap, breaking the degree-drift
   feedback loop by construction): degree drift eliminated exactly as
   designed (`max_i n_i^prune=3` always), exposure genuinely improved
   (`0.254→0.487`), but disconnection got WORSE (`80%→100%`) and FASTER
   (window 3 uniformly) — removing the "occasional loosening" degree
   drift provided let the same concentrated nodes drain via 2
   consecutive windows instead of needing inflation first.
7. `[A63]` — exact capacity audit on K1d: `H-A` CONFIRMED AGAIN —
   `CR*≈0.52`, real improvement over K1c but still far short of 0.95.

**Two independently-motivated cap formulations (current-degree,
reference-degree) both hit the identical structural wall at the exact-
optimum level.** Per this project's anti-fishing discipline, iterating
further cap variants (K1e, K1f, ...) without a qualitatively different
change would become exactly the unbounded search for a passing variant
`[A9]`'s "report the landscape, never chase a winning statistic" rule
forbids.

**Verdict: `FEASIBILITY REJECT`, explicitly NOT `BILUH hypothesis
FAIL`.** The distinction matters and is preserved deliberately: this
result says the specific ARCHITECTURE tested — rank all edges
independently by a state-derived desirability score, then delete the
top-ranked ones subject only to a per-node incidence constraint — is
poorly matched to node-correlated dynamics. It does **not** say
state-dependent structural plasticity is impossible, and it does
**not** revive or bear on `[A45]`'s separate open anomaly (Phase 11-12,
shuffled correlations beating real ones on curvature) or Phase 11-12's
own REJECT verdict, which stand on their own evidence.

**What is NOT killed:** the fast-dynamics backend, the damage/
corruption toolkit, `SeedManager`'s seed-stream discipline, the ICE-gate
methodology (Exposure/Connectivity/Cap-Activity), the exact-capacity-
audit technique itself (`observables/capacity_matching.py`, now a
tested, reusable module), and the permutation-equivariance red-team
protocol are all sound, validated, and reusable for whatever comes
next.

**New hypothesis this result motivates, NOT yet tested (see V5's own
pre-registration, `docs/v5_spec.md`):** the elementary structural
operation itself may be mis-specified. Independent edge DELETION,
decoupled from creation, is exactly the decomposition that manufactures
the star-collapse failure. A degree-preserving, connectivity-checked
edge SWAP (support rewiring) as the atomic operation removes the
failure mode by construction (no node's degree ever changes, hence no
incidence cap is even needed) rather than by constraining it after the
fact. This is a genuinely new, independently-motivated mechanism change
— not a post-hoc rescue of the rejected architecture — following
directly from the diagnosed failure mode (`[A57]`-`[A63]`), not from a
desire to force a PASS.

**Evidence:** [VERIFIED-bash] full chain `[A57]`-`[A63]`, this session's
transcript; recorded permanently per
`~/.claude/rules/falsification-ladder.md`'s null_results protocol in
